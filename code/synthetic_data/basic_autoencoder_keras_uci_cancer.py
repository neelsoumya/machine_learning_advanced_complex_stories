...
#
# Usage:
#   python3 basic_autoencoder_keras_uci_cancer.py 'breast-cancer-wisconsin_MOD_CURATED.data' 'uci_data_' 0.4
#
# Adapted from:
...
if __name__ == '__main__':
    ...
    plt.savefig(str_output_filename_prefix + 'autoencoder_balance_trainingset.eps', dpi=300)

    plt.figure(figsize=(8, 4))
    sns.countplot(x=y_test)
    plt.savefig(str_output_filename_prefix + 'autoencoder_balance_testset.eps', dpi=300)
    ...
    keras.utils.plot_model(model_autoencoder,
                           to_file=str_output_filename_prefix + 'autoencoder_keras_model.eps',
                           show_shapes=True,
    ...
    model_autoencoder.save(str_model_save_filename)
    ...
    # feed in autoncoder hidden dimensions
    ...
    # use autoencoder hidden dimensions
    fpr_rf, tpr_rf, threshold_rf = roc_curve(y_test, y_pred_rf)
    auc_rf = auc(fpr_rf, tpr_rf)

    print("AUC from hidden dimensions fed to a random forest model", auc_rf)
