...
#
# Usage:
#   python mixed_datatype_autoencoder_keras_MLP_multicategorical4_column_deleted_contrast_bootstrap.py df_med_suicide_features_lengthened_final_ml_f20_FULL_column_deleted.csv  drug_suidata_autoencoder_f20_MLP_FULL_column_deleted_bootstrap  0.5
#  
# Adapted from:
...
if __name__ == '__main__':
    ...
    list_auc = [] # list to store AUC values

    i_NUM_BOOTSTRAP = 1000 # 1000
    # start boostrapp AUC
    for _ in np.arange(0, i_NUM_BOOTSTRAP):
        ...
        # append AUC value to list which stores all bootstrapped values
        list_auc.append(auc_rf) # may need to cast to float
    ...
    print("\n ***************************************** \n")
    print("\n 95% CI for AUC bootstrapped \n", lb, ub)

    # vidualize boostrapped AUC   
    plt.figure()
    sns.distplot(list_auc)    
    plt.vlines(lb, 0, 1)
    plt.vlines(ub, 0, 1)
    plt.savefig("hist_auc_bootstrap.png", dpi=150, alpha=True)
    plt.close()
    
    plt.figure()
    plt.hist(list_auc)
    plt.savefig("hist_auc_bootstrap2.png", dpi=150, alpha=True)
    plt.close()
